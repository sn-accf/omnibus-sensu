    ...
    def extract
      ...
      if Ohai["platform"] == "windows"
        if downloaded_file.end_with?(*TAR_EXTENSIONS) && source[:extract] != :seven_zip
          ...
          shellout!("tar #{compression_switch}xf #{safe_downloaded_file.tr('\\', '/').sub(/^([A-Z]):/, '/\1').downcase} -C #{safe_project_dir.tr('\\', '/').sub(/^([A-Z]):/, '/\1').downcase}", returns: returns)
        ...
        shellout!("#{tar} #{compression_switch}xf #{safe_downloaded_file.tr('\\', '/').sub(/^([A-Z]):/, '/\1').downcase} -C #{safe_project_dir.tr('\\', '/').sub(/^([A-Z]):/, '/\1').downcase}")
      end
    end
